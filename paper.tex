...
Transcription is the first essential step of gene expression, in which a DNA template sequence is copied into a complementary single stranded RNA sequence by a `copy machine' called RNA polymerase. 
...
\draw[-latex] (origin) -- ++(0:5) node[below] {$\delta$}; 
\node at (2, -0.2) {1};
...
In \cite{DHOPRSST2018}, Demaine et al. proved that at delay 1 and arity 1, upon the seed of length $n$, a deterministic oritatami system cannot fold into any conformation strictly longer than $10n$. 
