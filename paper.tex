...
\begin{theorem}\label{thm:OS_to_2dTM}
	There exists a Turing machine with 2-dimensional tape that simulates any oritatami system $\Xi = (\Sigma, R, \delta, \alpha, \sigma, w)$ in time ??. 
\end{theorem}
\begin{proof}
	...
	We can check for each of these elongations whether its arity is at most $\alpha$ or not and whether it is $R$-valid or not in time $O(\delta)$. 
\end{proof}
