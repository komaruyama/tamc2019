...
\begin{proof}
%	The bead $w[i+1]$ consumes one hand and provide nothing, that is, $\#bc(C_{i}) \geq \#bc(C_{i+1})$.
%	The bead $w[i]$ consumes bond because $S[i] = b$.
%	See Fig.~\ref{TTT_tunnelC_enter_usingBond}.
%	The bead $w[i+2]$ is stabilized at a point of neighbors of $w[i]$.
%	If $w[i]$ consumes just one hand, $w[i]$ leaves a hand, that is, $\#bc(C_{i-1}) \geq \#bc(C_{i})$.
%	Then, $w[i+2]$ consumes $w[i]$'s hand.
%	Thus, $\#bc(C_{i-1}) > \#bc(C_{i+2})$.
%	On the other hand, if $w[i]$ forms two bonds, then even if $w[i+2]$ provides two free hands, $\#bc(C_{i-1}) - \#bc(C_{i}) = \#bc(C_{i+1}) - \#bc(C_{i+2})$.
%	Thus, $\#bc(C_{i-1}) > \#bc(C_{i+2})$.
...
Let $\Xi$ be a deterministic unary oritatami system of $\delta = 1, \alpha = 3$. It can yield only finite conformations of length is $2n + 14$.
\end{theorem}
...
\begin{figure}
  ...
  \centering
    ...
    \end{minipage}
    \end{tabular}
    \caption{The bead $w[i]$  the first bead outside $\hexagon^n_0$: $w[i+1]$ elongated to west (left), $w[i+1]$ elongated to east (right)}
    \label{fig:ttt_d2}
  \end{center}
\end{figure}
