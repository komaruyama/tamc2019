...
$S[i+2] \in \{ t_s, t_o \} $ means that $w[i+2]$ is stabilized by a tunnel section type of $S$ or $O$.
...
Hence, assume $w[i]$ does not form any bond at stabilization.
Then $w[i-1]$ is not on $n_1$ or $n_2$.

On the other hand, let us consider $S[i] = t$.
Beads which are shared by neighbors of $w[i-1]$ and $w[i]$ are occupied so that we consider only Fig.~\ref{TTT_tunnel_direction} (Left) and (Middle) case because (Right) case is not occupied them.
In the (Left) case, if $S[i] = t$, points $n_3$ and $n_4$ are occupied. Then, $S[i+1] = t$, contradiction.
In the (Middle) case, if $S[i] = t$, $n_4$ is free and $n_5$ is occupied because of $S[i+1] = b$.
This time, by $n_4$ has some free hands because at least three neighbors of $n_4$ are free so $w[i]$ has to be bound to $n_2$. Thus, $\#bc(C_{i-1}) > \#bc(C_{i})$.

%In the case of $t_0$, one of $n_3$ and $n_4$ is free because of $S[i+1] = b$. Thus, this case is only $S[i] = b$.
%
%
%In the case of $t_{\pm 60}$, if $S[i] = t$, the point $n_4$ is free and $n_5$ is occupied. This time, by $n_2$ has some free hands because at least three neighbors of $n_2$ are free so $w[i]$ has to be bound to $n_2$. Thus, $\#bc(C_{i-1}) > \#bc(C_{i})$.
%
%In the case of $t_{\pm120}$, the point of $w[i+1]$ is free before $w[i+1]$ is stabilized. Thus, this case is only $S[i] = b$.
%
