...
\begin{figure}[tb]
  \begin{center}
    ...
    \end{tabular}
    \caption{Direction into an entrance of a tunnel: (Left) straight, (Middle) obtuse, (Right) acute. The bead $w[i]$ is stabilized at an entrance and $w[i+1]$ is stabilized inside a tunnel.}
...
\begin{proof}
	...
	As prove in Lemma~\ref{TTT_exit}, $\#bc(C_{i-2}) \geq \#bc(C_{i})$.%the increase of binding capability by $w[i-1]$ and $w[i]$ is at most 0.
	...
	% The bead $w[i-2]$ must consume two bonds ; otherwise $w[i-2]$'s hands is used by $w[i]$.
	The bead $w[i-1]$ consumes one hand and provides nothing.
...
\begin{figure}[tb]
  ...
  \centering
	...
	\begin{minipage}{0.48\hsize}     
        ...
        \end{minipage}
        \end{tabular}
    \caption{Case of $S[i] = t_a$ and $S[i+2] = t_a$: (Right) Moreover $w[i]$ does not consume any bond.}
...
That is, this inequality involves two tunnels but its difference 2 enables us to consider that binding capability decreases by 1 through this tunnel. \qed
%According to Lemma~\ref{TTT_tunnelC_lemma}, on Statement 1, 2, $\#bc(C_{i-2}) > \#bc(C_{i+2})$ and if $i \leq j - 3$, $\#bc(C_{i+2}) \geq \#bc(C_{j})$.
...
\begin{figure}[tb]
  ...
  \centering
    ...
    \end{minipage}
    \end{tabular}
    \caption{The moment when the transcript steps outside $\hexagon^n_0$.}
    \label{fig:ttt_d2}
  \end{center}
\end{figure}
...
\begin{proof}
 We assume $w[i]$ is the first bead stabilized outside $\hexagon^n_0$.
 ...
 Until when a bead is stabilized outside $\hexagon^{n+1}_0$, binding capability never increases because of arity being 2 and of the Tunnel Troll Theorem.
 This means that, only at most $\#bc(C_0) \leq 2n$ times we can thus expand that hexagonal region.
 In other words, outside $\hexagon^{3n}_0$ the transcript cannot help but fold into the zig-zag conformation. \qed
