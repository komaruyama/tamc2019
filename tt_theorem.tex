...
Let $\Xi$ be a deterministic unary oritatami system of delay $\delta = 1$, arity $\alpha = 2$. If there are indices $i,j$ such that $S[i..j+1]$ is either $bbt^{(j-i-1)}b$ or $bt^\ell bt^{j-i-1-\ell}b$ for some $\ell$ and $t_a \in S[h+1..i]$, then $\#bc(C_{i-1}) > \#bc(C_j)$.
\end{lemma}
